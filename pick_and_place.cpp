...
void OpenManipulatorPickandPlace::demoSequence()
{
  ...
  case 6: // place pose
  if(pick_ar_id_ == 0){
    joint_angle.push_back( 1.606); //1.57
    joint_angle.push_back(-0.687); //-0.21
    joint_angle.push_back(0.515);
    joint_angle.push_back( 1.629);
    setJointSpacePath(joint_name_, joint_angle, 3.0);
    }
   else if (pick_ar_id_ == 1){
    joint_angle.push_back( 1.606); //1.57
    joint_angle.push_back(-0.587); //-0.21
    joint_angle.push_back(0.005);
    joint_angle.push_back( 1.781);
    setJointSpacePath(joint_name_, joint_angle, 3.0);
    }
   else if (pick_ar_id_ == 2){
    joint_angle.push_back( 1.606); //1.57
    joint_angle.push_back(-0.622); //-0.21
    joint_angle.push_back(-0.120);
    joint_angle.push_back( 1.863);
    setJointSpacePath(joint_name_, joint_angle, 3.0);
    }
   else if (pick_ar_id_ == 3){
    joint_angle.push_back( 1.606); //1.57
    joint_angle.push_back(-0.701); //-0.21
    joint_angle.push_back(-0.340);
    joint_angle.push_back( 1.880);
    setJointSpacePath(joint_name_, joint_angle, 3.0);
    }
    demo_count_ ++;
    break;
  case 7: // place the box
    ...
    if (pick_ar_id_ == 0)  kinematics_position.push_back(present_kinematic_position_.at(2)-0.03); // -0.076 MINDER HOOG IS DICHTER NAAR 0
    else if (pick_ar_id_ == 1)  kinematics_position.push_back(present_kinematic_position_.at(2)-0.021); //-0.041
    else if (pick_ar_id_ == 2)  kinematics_position.push_back(present_kinematic_position_.at(2)-0.0012); //-0.006
    else if (pick_ar_id_ == 3)  kinematics_position.push_back(present_kinematic_position_.at(2)-0.0012);
    kinematics_orientation.push_back(0.74);
    kinematics_orientation.push_back(0.00);
    kinematics_orientation.push_back(0.66);
    kinematics_orientation.push_back(0.00);
  ...
  case 9: // move up after place the box
    joint_angle.push_back( 1.57); //1.57
    joint_angle.push_back(-0.9); //-0.21
    joint_angle.push_back(-0.15);
    joint_angle.push_back( 1.590);
    setJointSpacePath(joint_name_, joint_angle, 2.0);
  //  kinematics_position.push_back(present_kinematic_position_.at(0));
  //  kinematics_position.push_back(present_kinematic_position_.at(1));
  ...
  case 10: // home pose
    ...
    else if (pick_ar_id_ == 2) pick_ar_id_ = 3;
    else if (pick_ar_id_ == 3)
    {
      pick_ar_id_ = 0;
      demo_count_ = 0;
...
int main(int argc, char **argv)
{
  ...
  ros::Timer publish_timer = node_handle.createTimer(ros::Duration(1)/*1000ms*/, &OpenManipulatorPickandPlace::publishCallback, &open_manipulator_pick_and_place);
